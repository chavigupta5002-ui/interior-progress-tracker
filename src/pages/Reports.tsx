...
import { buildDayReports, enumerateDateRange, todayDateKey } from '../lib/reportDays'
import {
  UNASSIGNED_KEY,
  buildItemIndex,
...
export function Reports() {
  ...
  const [singleDate, setSingleDate] = useState(() => todayDateKey())
  ...
  // Auto-generate today's report once, as soon as a property is known
  // (either from the ?propertyId= param or the default first property).
  // After this initial load, everything regenerates only on the
  // "Generate report" click, same as before.
  const autoGeneratedRef = useRef(false)
  useEffect(() => {
    if (autoGeneratedRef.current || !propertyId) return
